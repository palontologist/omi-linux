...
export function GeneralTab(): React.JSX.Element {
  ...
  const [ttsProvider, setTtsProvider] = useState<'web' | 'deepgram'>(
    () => getMonologurSettings().ttsProvider
  )
  ...
  const [personality, setPersonality] = useState(
    () => loadAgentSettings().personality || 'warm, curious, and helpful'
  )
  const [activationMode, setActivationMode] = useState<'wake-word' | 'always'>(
    () => loadAgentSettings().activationMode || 'wake-word'
  )
  const [clarificationEnabled, setClarificationEnabled] = useState(
    () => loadAgentSettings().clarificationEnabled !== false
  )
  const [llmProvider, setLlmProvider] = useState<'deepgram' | 'openai' | 'ollama' | 'local'>(
    () => loadAgentSettings().llmProvider || 'deepgram'
  )
        ...
        control={
          <div className="flex flex-col items-end gap-2">
              ...
              <button
                onClick={async () => {
                  if (agentActive) {
                    ...
                    const loadMemories = async (): Promise<
                      Array<{ content: string; category?: string }>
                    > => {
                      try {
                        const r = await omiApi.get('/v3/memories', {
            ...
            <div className="flex gap-2 text-xs">
              ...
              >
                <option value="wake-word" className="bg-neutral-900">
                  Say "{agentName}" to activate
                </option>
                <option value="always" className="bg-neutral-900">
                  Always respond
                </option>
              </select>
              <select
              ...
              >
                ...
                <option value="fr" className="bg-neutral-900">
                  Français
                </option>
                <option value="de" className="bg-neutral-900">
                  Deutsch
                </option>
                <option value="zh" className="bg-neutral-900">
                  中文
                </option>
              ...
              <div className="flex items-center gap-2 ml-2">
                <input
                  ...
                  className="rounded"
                />
                <label htmlFor="sign-language" className="text-xs text-white/60 whitespace-nowrap">
                  Sign Language
                </label>
              </div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    saveAgentSettings({
                      ...
                      personality,
                      activationMode,
                      clarificationEnabled: e.target.checked,
                      llmProvider,
                      llmModel,
                      llmBaseUrl
                    })
            ...
            <div className="flex gap-2 text-xs">
              <select
                value={llmProvider}
                onChange={(e) => {
                  const v = e.target.value as 'deepgram' | 'openai' | 'ollama' | 'local'
                  setLlmProvider(v)
                  // Sensible default base for the local route when none set yet.
                  const baseFor =
                    v === 'local'
                      ...
                      : v === 'ollama'
                        ...
                        : llmBaseUrl
                  const nextBase = llmBaseUrl || baseFor
                  if (v === 'local' && !llmBaseUrl) setLlmBaseUrl(nextBase)
                  saveAgentSettings({
                    agentName,
                    personality,
                    activationMode,
                    ...
                    llmProvider: v,
                    llmModel,
                    llmBaseUrl: nextBase || undefined
                  })
                  if (v === 'ollama' || v === 'local') {
                    window.omi.deepgramAgentOllamaCheck(nextBase || undefined).then((r) => {
              ...
              >
                ...
                <option value="openai" className="bg-neutral-900">
                  OpenAI
                </option>
                <option value="ollama" className="bg-neutral-900">
                  Ollama (local)
                </option>
                <option value="local" className="bg-neutral-900">
                  Local (OpenAI API: llama-server, LM Studio)
                </option>
              </select>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        saveAgentSettings({
                          ...
                          llmProvider,
                          llmModel,
                          llmBaseUrl: e.target.value || undefined
                        })
                      }}
                      placeholder="Base URL (http://localhost:8080/v1)"
                      className="w-56 rounded-md bg-white/10 px-2 py-1 text-white focus:outline-none"
                    />
                  )}
                  <button
                    onClick={async () => {
                      const r = await window.omi.deepgramAgentOllamaCheck(llmBaseUrl || undefined)
                      setOllamaStatus({ checked: true, ok: r.ok, models: r.models ?? [] })
                    }}
            ...
            <button
              onClick={async () => {
                ...
                if (segments.length === 0) {
                  setSummaryResult({
                    summary: 'No transcript yet. Start recording first.',
                    tasks: [],
                    keyPoints: []
                  })
                  return
